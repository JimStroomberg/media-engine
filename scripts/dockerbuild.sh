...
if [[ -z "${TAG}" ]]; then
  ...
  echo "  JETSON_PLATFORMS (default linux/arm64)" >&2
...
JETSON_PLATFORMS="${JETSON_PLATFORMS:-linux/arm64}"
PUSH="${PUSH:-true}"
SBOM="${SBOM:-true}"
PROV="${PROV:-true}"
...
CACHE_REF_JETSON="${IMAGE_REPO}:buildcache-jetson-xavier-nx"
...
echo ">> Building Jetson Xavier NX (${JETSON_PLATFORMS}) against the JetPack 5-compatible base..."
"${BUILD_CMD[@]}" \
  --platform "${JETSON_PLATFORMS}" \
  --cache-from "type=registry,ref=${CACHE_REF_JETSON}" \
  --cache-to   "type=registry,ref=${CACHE_REF_JETSON},mode=max" \
  -t "${IMAGE_REPO}:jetson-xavier-nx-${TAG}" \
  -t "${IMAGE_REPO}:jetson-xavier-nx-latest" \
  -f Dockerfile.jetson \
  .
