...
MEDIA_ENGINE_WORKER_IMAGE=jimstro/media-engine:0.5.0
...
```dotenv
MEDIA_ENGINE_WORKER_IMAGE=jimstro/media-engine:rk1-0.5.0
...
```dotenv
MEDIA_ENGINE_WORKER_IMAGE=jimstro/media-engine:jetson-xavier-nx-0.5.0
...
For the `0.5.0` transcode-profile contract, upgrade standalone workers before the control plane. A `0.5.0` worker accepts
both the older v1 options and the new v2 `quality_profile`; a `0.4.1` worker would ignore the new compression tier. Roll
workers one at a time and confirm each returns online before updating the control plane and its bundled worker.
